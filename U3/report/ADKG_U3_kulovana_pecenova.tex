...
Úloha \textbf{Digitální model terénu a jeho analýzy} se zabývá vytvořením aplikace, která Delaunayovou triangulací nad vstupní množinou bodů $P$ vytvoří trojúhelníkovou síť, pro kterou se lineární interpolací vypočítají vrstevnice. Aplikace dále počítá a vhodně vizualizuje sklon a expozici trojúhelníků ke světovým stranám.\\ 
...
V následující kapitole jsou porovnány části DMT vytvořeného aplikací a DMT, který byl již dříve vytvořen v SW \textit{Atlas}. Aplikace nezvládá zobrazovat současně vykreslené vrstevnice a sklon trojúhelníků. Ukázka výstupů je rozdělena do dvou snímků. 

\subsection{Celý model}
\begin{figure}[h!]
	\centering
	\includegraphics[width=11cm]{./pictures/kupec_dtm_contours.png}
	\includegraphics[width=11cm]{./pictures/kupec_dtm_slope.png}
	\caption{DMT vytvořený aplikací}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[width=15cm]{./pictures/atlas_dtm.png}
...
\subsection{Hřbetnice}



%vykreslení trojúhelníků, které tam  nechceme
%údolí
%silnice
\clearpage
...
Třída \textit{Algorithms} obsahuje metody pro výpočet Delaunayova triangulace a analýzu DTM.

\subsubsection*{delaunayTriangulation}
Metoda \textbf{delaunayTriangulation} vytváří nad množinou bodů Delaunayovy triangulaci. Na vstupu je vektor bodů typu \texttt{QPoint3D}, metoda vrací uspořádaný vektor hran \texttt{Edge}, které tvoří jednotlivé trojúhelníky.\\
...
Metoda \textbf{on\_slope\_button\_clicked} obarví trojúhelníky vygenerované Delaunayovou triangulací v odstínech šedi podle hodnoty sklonu daného trojúhelníku.
